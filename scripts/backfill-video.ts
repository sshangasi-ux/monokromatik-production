...
//
// The live pipeline already attaches video at generation time. This applies the
// SAME verified chain (publisher embed → judge-verified YouTube match → opt-in
// stock) to the existing back catalogue via the shared sourceArticleVideo().
// Idempotent: skips articles that already have a videoUrl unless --force.
// --dry-run reports what it would attach without writing, so you can review
