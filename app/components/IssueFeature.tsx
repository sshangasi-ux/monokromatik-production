    ...
    <div className="min-h-screen bg-mono-white">
      ...
      <header className="bg-mono-black text-mono-white py-16 md:py-24 border-b border-mono-white/15">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-xs tracking-[0.36em] font-display font-bold text-mono-amber mb-7">{franchise}</p>
          <h1 className="max-w-5xl text-5xl md:text-7xl font-display font-bold leading-[0.94]">{title}</h1>
          <p className="max-w-3xl mt-8 text-xl md:text-2xl text-mono-soft-white font-body leading-relaxed">{standfirst}</p>
      ...
      {next && (
        <section className="bg-mono-black text-mono-white py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row md:items-center justify-between gap-8">
            ...
            <Link href={next.href} className="inline-flex items-center gap-2 bg-mono-amber text-mono-white px-7 py-4 font-display font-bold">READ NEXT <ArrowRight size={18} /></Link>
          </div>
        </section>
      )}
    </div>
